...
function slideShow() {
  ...
  var globals = {
    slideDelay: 40000000, // The time interval between consecutive slides.
    fadeDelay: 5, // The time interval between individual opacity changes. This should always be much smaller than slideDelay.  
    wrapperID: "slideShowImages", // The ID of the <div> element that contains all of the <img> elements to be shown as a slide show.
    buttonID: "nextButton", // The ID of the <button> element that goes to next picture.
    buttonID2: "prevButton", // The ID of the <button> element that goes to previous picture.
    button1Text: "<--", // Text used in the slide show toggle button.
    button2Text: "-->", // Text used in the slide show toggle button.    
  ...
  if (globals.buttonObject) {
    globals.buttonObject.addEventListener('click', backSlides, false); // This callback is used to toggle the slide show on and off.
  } 
  if (globals.buttonObject2) {
    globals.buttonObject2.addEventListener('click', transitionSlides, false); // This callback is used to toggle the slide show on and off.
  } 
  startSlideShow();
  ...
  function initializeGlobals() {   
    ...
    globals.buttonObject2 = (document.getElementById(globals.buttonID2) ? document.getElementById(globals.buttonID2) : null);   
  ...
  function initializeSlideShowMarkup() {  
    ...
    if (globals.buttonObject) {
      globals.buttonObject.textContent = globals.button1Text;
    }
      if (globals.buttonObject2) {
      globals.buttonObject2.textContent = globals.button2Text;
    }
  } // initializeSlideShowMarkup
  ...
  function backSlides() {
    ...
    --(globals.slideIndex);
     ...
     var lastSlide = globals.slideImages.length -1; //index=0 is first slide. last is
     //document.write(slideCount);
    if (globals.slideIndex < 0) {
      globals.slideIndex =lastSlide;
    }
